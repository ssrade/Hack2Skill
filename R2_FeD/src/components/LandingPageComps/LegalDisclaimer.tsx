...
export const LegalDisclaimer = () => {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start gap-4 relative z-10">
          ...
          <div className="space-y-3  relative top-0 left-20 m-3 p-6">
            ...
            <h5 className="font-bold text-lg bg-gradient-to-r from-indigo-300 to-purple-400 bg-clip-text text-transparent">
              {inline('Important Legal Disclaimer')}
            </h5>

            {/* Replaced AlertDescription with p */}
            <p className="text-sm max-w-[50vw] text-slate-300">
              <span className="font-semibold  text-slate-100">{inline('This AI assistant provides')} <span className="underline">{inline('informational analysis only')}</span></span>
              {" "}{inline('and does')} <span className="font-semibold text-slate-100 underline">{inline('not constitute legal advice')}</span>.
              {inline('Always consult a qualified attorney before making legal decisions.')}
              {inline('The analysis may contain errors or omissions.')}
            </p>
            ...
            <div className="inline-flex items-center bg-gradient-to-r from-indigo-500 to-purple-600 text-white border-0 shadow-sm shadow-indigo-400/40 text-xs px-2.5 py-0.5 rounded-full">
              ...
              {inline('For Informational Purposes Only')}
            </div>
          </div>
        </div>
      </div>
      ...
      >
        {inline('This tool is designed to assist with document analysis but should not replace professional legal counsel.')}
      </motion.div>
    </motion.div>
  );
};